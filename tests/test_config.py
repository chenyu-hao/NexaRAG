...
class TestConfig:
    ...
    def test_memory_and_context_compaction_defaults_exist(self, config):
        assert config.enable_session_event_recording is True
        assert config.daily_memory_filename == "每日记忆.md"
        assert config.write_daily_summary_file is True
        assert config.enable_long_term_memory_promotion is True
        assert config.enable_context_compaction is True
        assert config.context_keep_head_turns == 2
        assert config.context_keep_tail_turns == 6
        assert config.tool_result_placeholder_max_chars == 200
        assert config.middle_summary_max_chars == 1200

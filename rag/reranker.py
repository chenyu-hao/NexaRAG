"""DashScope rerank support for the RAG module."""
import logging
...
class Reranker:
    ...
    def rerank(self, query: str, documents: list[dict], top_k: int = None) -> list[dict]:
        ...
        if not documents:
            return []
        if not self.model:
            logger.info("Rerank skipped because rerank_model is not configured")
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning("Rerank failed: %s", resp.message)
                return documents[:top_k]
            reranked = []
            ...
            logger.error("Rerank failed with exception: %s", e)
            return documents[:top_k]
